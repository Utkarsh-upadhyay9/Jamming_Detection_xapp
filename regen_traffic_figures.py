#!/usr/bin/env python3
"""
Regenerate traffic flow figures with MATLAB style (simplified labels).
"""
...
# Load results
results = json.load(open('results/traffic_flow_experiment/traffic_flow_results.json'))
...
fig_dir = Path('figs/traffic_flow_experiment')
...
profile_names = ['High Flow', 'Low Flow']  # Removed throughput labels
...
plt.suptitle('Traffic Flow Experiment: Performance Analysis', 
            fontsize=14, fontweight='bold', y=0.98)
plt.tight_layout()
plt.savefig(fig_dir / 'traffic_flow_performance.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.suptitle('Traffic Flow Experiment: Confusion Matrices', 
            fontsize=14, fontweight='bold', y=0.98)
plt.tight_layout()
plt.savefig(fig_dir / 'traffic_flow_confusion.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"✅ Traffic flow figures regenerated (MATLAB style)")
