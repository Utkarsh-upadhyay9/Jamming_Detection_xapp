"""
Data processing and feature engineering module for jamming detection.
Based on the research paper's 15 engineered features.
"""
...
class JammingDataProcessor:
    """Process raw network data and engineer features for jamming detection."""
    ...
    def __init__(self, normalization_method: str = 'zscore'):
        ...
        Args:
            normalization_method: Method for feature normalization ('zscore', 'minmax', 'robust')
        """
        ...
        # Feature statistics for reference values
        self.feature_stats = {}
    ...
    def _get_feature_names(self) -> List[str]:
        """Get the names of all 15 engineered features."""
        return [
            # Signal Quality Indicators (5 features)
            'sinr_mean', 'sinr_std', 'rsrp_mean', 'rsrp_std', 'rsrq_mean',
            
            # Traffic Characteristics (4 features)  
            ...
            # Resource Utilization (4 features)
    ...
    def load_dataset(self, normal_traffic_path: str, jamming_attacks_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Load the jamming detection dataset.
        ...
        Returns:
            Tuple of (normal_data, jamming_data)
        """
    ...
    def prepare_dataset(self, normal_data: pd.DataFrame, jamming_data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Args:
            normal_data: Normal traffic DataFrame
            jamming_data: Jamming attacks DataFrame
    ...
    def engineer_features(self, raw_data: Dict[str, np.ndarray]) -> np.ndarray:
        """
        Engineer the 15 features from raw MAC layer metrics.
        
        Args:
            raw_data: Dictionary containing raw network metrics
    ...
    def _calculate_sinr_std(self, data: Dict[str, np.ndarray]) -> float:
        """Calculate SINR standard deviation."""
        sinr_values = data.get('sinr', np.array([0]))
    ...
    def _calculate_inter_arrival_variance(self, data: Dict[str, np.ndarray]) -> float:
        """Calculate inter-packet arrival time variance."""
    ...
    def _calculate_buffer_occupancy(self, data: Dict[str, np.ndarray]) -> float:
        """Calculate buffer occupancy ratio."""
    ...
    def _calculate_prb_utilization_ul(self, data: Dict[str, np.ndarray]) -> float:
        """Calculate uplink PRB utilization."""
    ...
    def _calculate_prb_utilization_dl(self, data: Dict[str, np.ndarray]) -> float:
        """Calculate downlink PRB utilization."""
    ...
    def _calculate_grant_count(self, data: Dict[str, np.ndarray]) -> float:
        """Calculate scheduling grant count."""
    ...
    def detect_periodic_patterns(self, frequency_sequence: np.ndarray, 
        ...
        """
        Detect periodic patterns in frequency sequence for sweep jamming.
        
        Args:
            frequency_sequence: Sequence of frequency measurements
            threshold: Threshold for periodicity detection
    ...
    def calculate_temporal_correlation(self, features: np.ndarray, 
        ...
        """
        Calculate temporal correlation for intelligent jamming detection.
        
        Args:
            features: Feature time series
    ...
    def calculate_adaptivity_score(self, features: np.ndarray,
        ...
        """
        Calculate behavior adaptivity for intelligent jamming detection.
        
        Args:
            features: Feature values
            traffic_patterns: Observed traffic patterns
        ...
        # Calculate correlation between features and traffic patterns
        try:
    ...
    def preprocess_for_training(self, features: np.ndarray, labels: np.ndarray,
        ...
        """
        Preprocess data for training with train/test split.
    ...
    def simulate_network_environment(self, base_features: np.ndarray, 
        ...
        """
        Simulate different network environments by adding noise and interference.
        
        Args:
            base_features: Base feature values
            environment: Environment type ('ideal', 'moderate', 'realistic')
            
        Returns:
            Modified features for the specified environment
        """
        ...
        if environment == 'ideal':
            ...
            # Moderate noise and interference
            noise_scale = 0.05
        elif environment == 'realistic':
            # High noise, interference, and multipath effects
            noise_scale = 0.1
        else:
        ...
        if environment == 'realistic':
            # Add multipath fading effects to SINR and RSRP
    ...
    def create_feature_windows(self, features: np.ndarray, 
        ...
        """
        Create sliding windows of features for temporal analysis.
    ...
    def get_feature_importance_analysis(self, features: np.ndarray, 
        ...
        # Calculate mutual information and correlation for each feature
        for i, feature_name in enumerate(self.feature_names):
            feature_values = features[:, i]
            
            # Calculate variance (higher variance might be more informative)
            variance_score = np.var(feature_values)
            
            # Calculate class separability using F-statistic
            unique_labels = np.unique(labels)
